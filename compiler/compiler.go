...
func New() *Compiler {
	return &Compiler{instructions: code.Instructions{}, constants: []object.Object{}, lastInstruction: EmittedInstruction{}, previousInstruction: EmittedInstruction{}, symbolTable: NewSymbolTable()}
}
...
func (c *Compiler) Compile(node ast.Node) error {
	...
	case *ast.IfExpression:
		...
		if c.lastInstructionIsPop() { c.removeLastPop() }
...
func (c *Compiler) Bytecode() *Bytecode { return &Bytecode{Instructions: c.instructions, Constants: c.constants} }
...
func (c *Compiler) addInstruction(ins []byte) int { pos := len(c.instructions); c.instructions = append(c.instructions, ins...); return pos }

func (c *Compiler) setLastInstruction(op code.Opcode, pos int) { previous := c.lastInstruction; last := EmittedInstruction{Opcode: op, Position: pos}; c.previousInstruction = previous; c.lastInstruction = last }
func (c *Compiler) lastInstructionIsPop() bool { return c.lastInstruction.Opcode == code.OpPop }
...
func (c *Compiler) changeOperand(opPos int, operand int) { op := code.Opcode(c.instructions[opPos]); newInstruction := code.Make(op, operand); c.replaceInstruction(opPos, newInstruction) }
